...
- Escolha o tamanho apropriado para o contexto da interface
	...
	argTypes: {
		...
		onPageChange: {
			description: 'Função chamada quando o usuário navega para uma página diferente',
			action: 'page changed',
		...
		showFirstLast: {
			description: 'Exibe botões para navegar diretamente à primeira e última página',
		...
		return (
			<div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
				<div>
					...
					<p>Mostrando {startItem}-{endItem} de {totalItems} itens</p>
				</div>
					...
					<table style={{ width: '100%', borderCollapse: 'collapse' }}>
						...
						<tbody>
							{Array.from({ length: Math.min(itemsPerPage, totalItems - startItem + 1) }, (_, i) => {
								...
								return (
									<tr key={itemId}>
										<td style={{ padding: '12px', borderBottom: '1px solid #f1f3f4' }}>{itemId}</td>
										<td style={{ padding: '12px', borderBottom: '1px solid #f1f3f4' }}>Usuário {itemId}</td>
										<td style={{ padding: '12px', borderBottom: '1px solid #f1f3f4' }}>usuario{itemId}@exemplo.com</td>
									</tr>
								);
							})}
						</tbody>
	...
	render: () => (
		<div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
			...
			<div>
				...
				<Pagination currentPage={3} totalPages={8} variant="outlined" onPageChange={() => {}} />
			</div>
			
			<div>
				<h3 style={{ marginBottom: '16px' }}>Minimal</h3>
				<Pagination currentPage={3} totalPages={8} variant="minimal" onPageChange={() => {}} />
			</div>
		</div>
	),
	parameters: {
	...
	render: () => (
		<div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
			<div>
				...
				<Pagination currentPage={3} totalPages={8} size="small" onPageChange={() => {}} />
			</div>
			
			<div>
				<h3 style={{ marginBottom: '16px' }}>Medium (Default)</h3>
				<Pagination currentPage={3} totalPages={8} size="medium" onPageChange={() => {}} />
			</div>
			
			<div>
				<h3 style={{ marginBottom: '16px' }}>Large</h3>
				<Pagination currentPage={3} totalPages={8} size="large" onPageChange={() => {}} />
			</div>
		</div>
	),
	parameters: {
