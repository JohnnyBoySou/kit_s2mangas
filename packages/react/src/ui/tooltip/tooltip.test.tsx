...
describe('Tooltip', () => {
	...
	it('hides tooltip on blur', async () => {
		render(
			<Tooltip content="Tooltip content" trigger="focus" showDelay={0} hideDelay={0}>
				<button>Trigger</button>
			</Tooltip>
		);
	...
	it('toggles tooltip on click', async () => {
		render(
			<Tooltip content="Tooltip content" trigger="click" showDelay={0} hideDelay={0}>
				<button>Trigger</button>
			</Tooltip>
		);
	...
	it('renders with different positions', async () => {
		...
		for (const position of positions) {
			const { unmount } = render(
				<Tooltip content="Tooltip content" position={position} showDelay={0} testID={`tooltip-${position}`}>
					<button>Trigger</button>
				</Tooltip>
			);
	...
	it('renders with different sizes', async () => {
		...
		for (const size of sizes) {
			const { unmount } = render(
				<Tooltip content="Tooltip content" size={size} showDelay={0} testID={`tooltip-${size}`}>
					<button>Trigger</button>
				</Tooltip>
			);
	...
	it('renders with different variants', async () => {
		...
		for (const variant of variants) {
			const { unmount } = render(
				<Tooltip content="Tooltip content" variant={variant} showDelay={0} testID={`tooltip-${variant}`}>
					<button>Trigger</button>
				</Tooltip>
			);
	...
	it('applies maxWidth style', async () => {
		render(
			<Tooltip content="Tooltip content" maxWidth={200} showDelay={0} testID="tooltip">
				<button>Trigger</button>
			</Tooltip>
		);
	...
	it('applies testID', async () => {
		render(
			<Tooltip content="Tooltip content" testID="custom-tooltip-id" showDelay={0}>
				<button>Trigger</button>
			</Tooltip>
		);
	...
	it('applies className', async () => {
		render(
			<Tooltip content="Tooltip content" className="custom-class" showDelay={0} testID="classed-tooltip">
				<button>Trigger</button>
			</Tooltip>
		);
	...
	it('applies style', async () => {
		render(
			<Tooltip content="Tooltip content" style={{ border: '2px solid blue' }} showDelay={0} testID="styled-tooltip">
				<button>Trigger</button>
			</Tooltip>
		);
	...
	it('renders JSX content', async () => {
		render(
			<Tooltip content={<div>JSX <strong>content</strong></div>} showDelay={0}>
				<button>Trigger</button>
			</Tooltip>
		);
